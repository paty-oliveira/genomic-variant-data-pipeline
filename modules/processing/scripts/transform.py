...
def main():
    ...
    try:
        ...
        raise RuntimeError(f"Missing required job argument: {error}") from error
    ...
    try:
        logger.info(f"Transforming the XML file: {object_key} stored in {bucket_name} S3 bucket.")
        ...
        logger.info("Updating Iceberg table")
        ...
        logger.error(f"An error occurred processing s3://{bucket_name}/{object_key}: {error}")
        raise
